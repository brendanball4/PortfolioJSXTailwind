    ...
    <div className="flex flex-col lg:flex-row min-h-screen max-w-[1600px] mx-auto w-full px-5 sm:px-8 lg:px-10">
      ...
      <aside className="w-full lg:w-[33rem] shrink-0 flex flex-col lg:px-4 pt-10 lg:my-40 lg:border-r border-stone-300 dark:border-stone-700">
        ...
        <div className="lg:hidden flex flex-col gap-4 pb-6 border-b border-stone-300 dark:border-stone-700">
          ...
          <label className="flex flex-col gap-1">
            ...
            <select
              ...
              className="w-full border border-stone-300 dark:border-stone-600 bg-white dark:bg-stone-800 text-stone-900 dark:text-white px-3 py-2 text-base"
            >
              {projects.map(p => (
        ...
        <nav className="hidden lg:flex flex-col gap-1 flex-1 justify-center mb-24">
          ...
          >
            <FiHome className="inline-block mr-2 mb-0.5" />Home
            <span className="absolute bottom-0 left-0 h-[2px] bg-accent-500 dark:bg-accent-400 transition-all duration-300 w-0 group-hover:w-full" />
          </Link>
          ...
          {projects.map(p => (
            ...
            >
              {p.title}
              <span className={`
                absolute bottom-0 left-0 h-[2px] bg-accent-500 dark:bg-accent-400 transition-all duration-300
        ...
        <main className="flex-1 flex flex-col px-0 lg:px-16 pt-10 lg:pt-[18vh] pb-16 overflow-y-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
            ...
            {project.link && (
              <a
                ...
                className="group flex items-center gap-2 text-stone-500 dark:text-stone-400 hover:text-stone-900 dark:hover:text-white transition-colors sm:ml-6 shrink-0"
              >
                ...
                <FiArrowUpRight className="w-3.5 h-3.5 opacity-0 -translate-x-0.5 translate-y-0.5 group-hover:opacity-100 group-hover:translate-x-0 group-hover:translate-y-0 transition-all duration-200" />
              </a>
            )}
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {project.stack.map(item => (
              <span key={item} className="px-3 py-1 text-sm bg-stone-200 dark:bg-stone-700 text-stone-800 dark:text-stone-200 border border-stone-300 dark:border-stone-600">
                {item}
              </span>
            ))}
          </div>
